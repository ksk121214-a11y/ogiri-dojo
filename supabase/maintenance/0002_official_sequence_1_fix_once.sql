...
-- ============================================================
-- 本番のプロフィール構成が「一般会員0件」という前提から変わり、実際には
-- 通常プロフィール17件（admin1件・友達含む一般16件）が存在することが判明した。
-- このファイルの安全ガード（ガード2：一般会員0件）は現状の本番と一致しない
-- ため、このまま実行すると必ず安全ガードで停止する。
--
-- 安全ガードを外して無理に実行してはいけない。ガードは「想定と違う状態で
-- 誤実行しない」ための唯一の防御であり、削除・緩和は事故の直接原因になる。
--
...
-- このファイル（0002）はその設計の元になった参考実装として残しているだけで、
-- 現状の本番へ直接使うものではない。
-- ============================================================
