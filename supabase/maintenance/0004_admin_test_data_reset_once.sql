...
-- 実際には通常プロフィール17件（admin1件・ボット14件・友達1件を含む一般16件）
-- が存在することが判明した。このファイルはadmin（role='admin'）だけを対象に
-- しており、ボット14件・友達1件のポイント・投稿は初期化対象外のまま残って
-- しまう（対象漏れ）。加えてこのファイルの安全ガード（一般会員0件）は現状の
-- 本番と一致せず、そのまま実行すると必ず安全ガードで停止する。
--
