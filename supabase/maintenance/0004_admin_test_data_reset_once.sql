-- ============================================================
-- ⚠️ 現在の本番環境では使用禁止（2026-09-22時点）。
-- ============================================================
-- 本番のプロフィール構成が「通常会員はadminだけ」という前提から変わり、
-- 実際には通常プロフィール17件が存在することが判明した。内訳は
--   - 運営者本人のアカウント：2件（うちrole='admin'は1件、もう1件は運営者が
--     別の通常会員として使っているアカウントでrole='user'）
--   - ボット：14件
--   - 友達：1件
-- （admin1件・非admin16件）。このファイルはadmin（role='admin'）だけを対象に
-- しており、運営者のもう1アカウント・ボット14件・友達1件（=非adminの16件）
-- のポイント・投稿は初期化対象外のまま残ってしまう（対象漏れ）。加えて
-- このファイルの安全ガード（一般会員0件）は現状の本番と一致せず、そのまま
-- 実行すると必ず安全ガードで停止する。
--
-- 安全ガードを外して無理に実行してはいけない。また「対象をadmin以外にも
-- 広げるために」ガードだけを緩めて使い回すことも禁止（対象の特定条件ごと
-- 作り直す必要があるため）。
--
-- 現在の本番環境に対しては、代わりに対象限定版の
--   supabase/maintenance/0006_all_normal_profiles_prelaunch_reset_once.sql
-- （事前確認は0005_all_normal_profiles_reset_check_readonly.sql）を使うこと。
-- このファイル（0004）はその設計の元になった参考実装として残しているだけで、
-- 現状の本番へ直接使うものではない。
-- ============================================================
