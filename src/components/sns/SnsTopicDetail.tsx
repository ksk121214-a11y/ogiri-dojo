...
export default function SnsTopicDetail({ topicId }: { topicId: string }) {
  ...
  // 2026-09-13（再々レビュー対応）：ゲストは回答できない仕様のため、投稿入力欄・
  // 送信ボタン自体を表示しない（押せるのに拒否される体験を避ける。閲覧・
  // いいね数の表示は引き続き許可する）。
          ...
          return (
            ...
            >
              ...
              {isGuest ? (
                ...
                // （閲覧は引き続き許可する）。
                <span className="mt-1 flex shrink-0 items-center gap-1 self-start rounded-full border border-[var(--ink)]/15 px-3 py-1.5 font-sans text-xs font-bold text-[var(--ink)]/50">
