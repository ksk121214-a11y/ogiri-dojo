...
// live-design-preview/AudienceAnsweringViewPreview.tsx（1個目のデザイン確認）の
// レイアウト・ゲームロジックはそのまま、見た目だけ大喜利素材2のネオン系に差し替えたコピー。
export default function AudienceAnsweringViewPreview2() {
  ...
  const myScore = useLiveDesignPreviewStore2((s) => s.myScore);
  const submitMyScore = useLiveDesignPreviewStore2((s) => s.submitMyScore);
  const sendTsukkomi = useLiveDesignPreviewStore2((s) => s.sendTsukkomi);
      ...
      <div className="grid h-full w-full grid-cols-1 grid-rows-[auto_auto_minmax(140px,1fr)_auto] gap-1 overflow-visible">
        ...
        <div className="relative z-20 -mt-[32px] flex min-h-0 flex-col items-center gap-2 overflow-visible">
          ...
          {/* 舞台視点には「残り○回」の行があり、客席視点にはそれがない。この行の
              ...
              同じ高さの不可視スペーサーで高さだけ揃える(1個目と同じ理由)。 */}
              ...
              >
                <div className="flex h-full w-full flex-col items-center gap-2">
                  ...
                  {scoreError && scoreError.answerId === displayedAnswer.id && (
                    <p className="font-sans text-xs text-red-400">{scoreError.message}</p>
                  )}
                </div>
              </motion.div>
            )}
