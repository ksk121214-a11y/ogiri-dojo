        ...
        <AnimatePresence mode="wait">
          {step === 0 && bestAnswer && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              className="w-full rounded-2xl border border-dojo-cheer-pink/60 bg-dojo-backstage-navy p-6 shadow-[0_0_40px_rgba(255,111,165,0.3)]"
            >
              <p className="font-sans text-xs tracking-widest text-dojo-cheer-pink">
                本日のベストアンサー
              </p>
              <p className="mt-3 font-sans text-xs text-dojo-curtain-gold">
          ...
          {step >= 1 && step <= 3 && top3.length >= step && (
            <motion.div
              ...
              className="w-full rounded-2xl border border-dojo-curtain-gold/70 bg-dojo-backstage-navy p-8 text-center shadow-[0_0_50px_rgba(232,184,76,0.35)]"
            >
              <p
              ...
              <p className="mt-3 font-sans text-lg font-bold text-dojo-washi-white">
                ...
                {ranking.map((r, idx) => (
                  <div
                    ...
                    className={`flex items-center justify-between rounded-lg border px-3 py-2 text-sm ${
                      r.participant.id === MY_PARTICIPANT_ID
                        ? "border-dojo-curtain-gold bg-dojo-backstage-navy"
                        : "border-dojo-gray-purple/20 bg-dojo-backstage-navy/50"
                    }`}
                  >
                    <span>
                      {idx + 1}位 {r.participant.displayName}
                      {r.participant.id === MY_PARTICIPANT_ID ? "（あなた）" : ""}
                    </span>
