...
//   実テストで「審査に時間がかかっている間に90秒が経過し、その後の送信がRLSで
...
// 一時停止で消費される時間ぶんが締切に反映されず、実際にはまだ90秒経っていないのに
...
async function runBotBehavior() {
  ...
  // 90秒の回答フェーズ中に数回程度発生する頻度を狙っている（2026-08-19：2%→5%に引き上げ）。
